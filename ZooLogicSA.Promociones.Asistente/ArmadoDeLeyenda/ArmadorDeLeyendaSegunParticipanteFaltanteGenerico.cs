...
namespace ZooLogicSA.Promociones.Asistente.ArmadoDeLeyenda
{
	public class ArmadorDeLeyendaSegunParticipanteFaltanteGenerico : IArmadorDeLeyendaSegunParticipanteFaltante
	{
        ...
        public string ObtenerLeyendaSegunRegla( ParticipanteFaltante participante, InformacionPromocionIncumplida info )
		{
			...
			// Obtener un participante del mismo tipo que se está evaluando.
			// No se usa ObtenerParticipantesSegunClave("COMPROBANTE") porque ParticipanteXml
			// requiere que la clave exista en ConfiguracionesPorParticipante, y COMPROBANTE no está.
			IParticipante cualquierParticipante = null;
			if (info.Comprobante != null && !string.IsNullOrEmpty(participante.Participante.Codigo))
			{
				var participantesItem = info.Comprobante.ObtenerParticipantesSegunClave(participante.Participante.Codigo);
				if (participantesItem != null && participantesItem.Any())
				{
					cualquierParticipante = participantesItem[0];
				}
			}
