...
namespace ZooLogicSA.Promociones.Asistente
{
    public class KontrolerAsistente
    {
        ...
        private estado ObtenerEstado( InformacionPromocionIncumplida info )
        {
            if (info != null)
            {
                // Si SatisfechoEfectivo fue establecido (>= 0) por el calculador de incumplimiento,
                // usarlo para determinar el color: 0 = gris (no apto), > 0 = amarillo (parcial).
                if (info.SatisfechoEfectivo >= 0)
                {
                    return info.SatisfechoEfectivo > 0 ? estado.Parcial : estado.Incumplida;
                }
        ...
        private Bitmap ObtenerDibujoDesdeEstado( estado estadoPromocion )
        {
            switch ( estadoPromocion )
            {
                case estado.Cumplida:
                    return Properties.Resources.Verde;
                case estado.Parcial:
                    return Properties.Resources.Amarillo;
                default:
        ...
        {
            return this.ObtenerDibujoDesdeEstado( this.ObtenerEstado( info ) );
        }
