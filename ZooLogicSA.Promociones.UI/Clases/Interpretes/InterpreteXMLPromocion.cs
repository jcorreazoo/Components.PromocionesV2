...
namespace ZooLogicSA.Promociones.UI.Clases.Interpretes
{
    public class InterpreteXMLPromocion
    {
        ...
        public void Ejecutarproceso( IControllerPromocion control, string idPromocion )
        {
            ...
            this.SetearAplicacionProductosIguales();
        ...
        public static int ObtenerAplicacionProductosIgualesDesdeXML( string XML )
        {
            XmlDocument xmlDoc = new XmlDocument();
            try
            {
                xmlDoc.LoadXml( XML );
                XmlNode nodo = xmlDoc.SelectSingleNode( cNombreNodoPromocion + "/AplicacionProductosIguales" );
                if ( nodo != null && !string.IsNullOrEmpty( nodo.InnerText ) )
                {
                    int intValue;
                    if ( int.TryParse( nodo.InnerText, out intValue ) )
                        return intValue;
                    try
                    {
                        return (int)Enum.Parse( typeof( AplicacionProductosIgualesType ), nodo.InnerText );
                    }
                    catch { }
                }
            }
...
//            ObtenerCuotasSinRecargo()
        ...
        private void SetearAplicacionProductosIguales()
        {
            ManagerReglas.Promo.AplicacionProductosIguales = this.control.ObtenerAplicacionProductosIguales();
        }
