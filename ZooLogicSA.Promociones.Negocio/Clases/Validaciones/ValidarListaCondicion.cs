...
namespace ZooLogicSA.Promociones.Negocio.Clases.Validaciones
{
    public class ValidarListaCondicion : IValidacionPromocion
    {
        ...
        public bool Validar( params object[] parametros )
        {
            ...
            if (validar)// me fijo que no haya mas de un atributo Total
            {
                if (!this.ValidarMenosDeUnAtributoTotal(parametros))
                {
                    validar = false;
                    this._mensajeError = "No se puede ingresar más de un atributo Total para cada condición.";
                }
            }
            return validar;
        ...
        private bool ValidarMenosDeUnAtributoTotal(object[] parametros)
        {
            bool retorno = true;
            string[] arr;
            ...
            foreach (string CadaCondicion in parametros)
            {
                arr = DevolverCondicionDescompuestaPorTotal(CadaCondicion);
                if (arr != null)
                {
                    if (arr.Length > 2)
                    {
        ...
        private string[] DevolverCondicionDescompuestaPorTotal(string condicion)
        {
