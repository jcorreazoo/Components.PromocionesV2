...
namespace ZooLogicSA.Promociones
{
    public class MotorPromociones : IMotorPromociones
    {
        ...
        public List<InformacionPromocion> AplicarPromociones( string idProceso, List<string> promos )
        {
            ...
            // Aplicar las promos de más restrictiva a menos restrictiva respecto a
            // AplicacionProductosIguales, para que las que exigen combinación específica
            // o artículo específico consuman primero antes que las de mezcla libre.
            // Orden: PorArticuloYCombinacion (2) → PorArticulo (1) → NoAplicar (0).
            List<string> promosOrdenadas = promos
                .OrderByDescending( codigo =>
                {
                    Promocion p = this.ObtenerPromocion( codigo );
                    return p != null ? (int)p.AplicacionProductosIguales : 0;
                } )
                .ToList();

            promosOrdenadas.ForEach( codigoPromocion => retorno.AddRange( this.EvaluarYAplicarPromocion( comprobante, codigoPromocion ) ) );
		...
		public List<InformacionPromocion> EvaluarYAplicarPromocion( IComprobante comprobante, string promo, TestigoCancelacion cancelacion )
		{
            ...
            if ( promocion != null && comprobante != null )
            {
                ...
                do
                {
                    ...
                    var idsBeneficiarios = new System.Collections.Generic.HashSet<string>(
                        promocion.Participantes.Where( p => p.Beneficiario ).Select( p => p.Id ) );
                    if ( respuesta.Coincidencias.Count( c => !idsBeneficiarios.Contains( c.IdParticipanteEnRegla ) )
                         == promocion.Participantes.Count( p => !p.Beneficiario ) )
                    {
        ...
        private Promocion NormalizarPromocionObtenida( Promocion promoObtenida )
        {
            ...
            foreach ( ParticipanteRegla participante in promoObtenida.Participantes )
            {
                    ...
                    && nuevosParticipantes.Exists( p => p.Beneficiario == participante.Beneficiario && p.Codigo == participante.Codigo ) )
                {
                    ...
                    {
                        string atributoCombinacion = this.configuracionComportamiento.ConfiguracionesPorParticipante[partParaAnalizar.Codigo].Combinacion;
                        bool ignorarCombinacion = promoObtenida.AplicacionProductosIguales == ZooLogicSA.Promociones.FormatoPromociones.AplicacionProductosIgualesType.PorArticulo
                                                  && !string.IsNullOrEmpty(atributoCombinacion);
                        ...
                        var paraAnalizar = ( from x in partParaAnalizar.Reglas
                                             ...
                                             && !(ignorarCombinacion && x.Atributo == atributoCombinacion)
                        ...
                        if ( paraAnalizar.Count() == participanteActual.Count() && paraAnalizar.SequenceEqual( participanteActual ) )
                        {
                            ...
                            if ( ignorarCombinacion )
                            {
                                Regla reglaCombinacion = partParaAnalizar.Reglas.FirstOrDefault( r => r.Atributo == atributoCombinacion );
                                if ( reglaCombinacion != null )
                                {
                                    reglaCombinacion.Comparacion = ZooLogicSA.Promociones.FormatoPromociones.Factor.DebeSerContieneA;
                                    reglaCombinacion.Valor = string.Empty;
                                }
                            }
