...
- **Archivado del código pre-reescritura:** ANTES del force-push de historial limpio, se crea y se pushea un **tag `snapshot-inicial`** apuntando al estado actual (todo el código viejo + relevamiento). Ese tag **NO se borra**, así el código queda consultable (`git grep <patrón> snapshot-inicial`, `git show snapshot-inicial:<ruta>`, o `git worktree add ../snapshot-wt snapshot-inicial`) sin ensuciar el proyecto nuevo (build/Spec Kit/tooling no lo ven). Elegido sobre la alternativa de una carpeta `.promociones.old/` dentro del repo. Se evita el nombre `legacy` para no confundir con la versión de **producción** (que vive en OTRO repo intocable) ni con el sector "Legacy" de la empresa.
